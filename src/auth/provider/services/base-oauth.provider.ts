...
export class BaseOAuthProvider {
    ...
    public getAuthUrl() {
        const query = new URLSearchParams({
            ...
            redirect_uri: this.getRedirectUrl(), // перенаправление после авторизации 
    ...
    public async findUserByCode(code): Promise<TypeUserInfo> {
        ...
        const tokeRequest = await fetch(this.options.access_url, {
            method: 'POST',
            body: tokenQuery,
            headers: {
        ...
        if (!tokeRequest.ok) {
            throw new BadRequestException(
                `Не удалось полученить пользователя с ${this.options.profile_url}. Проверьте правильность токена доступа.`
            )
        }

        const tokens = await tokeRequest.json()
